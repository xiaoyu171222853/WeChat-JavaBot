...
public class WeChatDllServiceImpl implements WeChatDllService {
    ...
    public WxPpWcfSendTextMsgResp sendTextMsg(WxPpWcfSendTextMsgReq request) {
        ...
        Wcf.TextMsg textMsg = Wcf.TextMsg.newBuilder().setMsg(atUserNickName + request.getMsgText()).setReceiver(request.getRecipient()).setAters(atUser).build();
    ...
    public boolean isAtMeMsg(String wxMsgXml, String wxMsgContent) {
        ...
        if (wxMsgXml.contains("atuserlist")){
            return wechatSocketClient.isAtMeMsg(wxMsgXml,wxMsgContent,wechatSocketClient.getSelfWxId());
        }else {
            return false;
        }
    }
    ...
    public String dealAtUser(List<String> atUsers) {
        ...
        if (!CollectionUtils.isEmpty(atUsers)) {
            ...
            for (String atUser : atUsers) {
                String nickName = queryNickNameByUserName(atUser);
                atUserStr.append("@").append(nickName);
            }
        }
